...
function normalizeIDs(test) {
    test.individus.forEach(function(individu) {
    ...
    Object.keys(EXPORTED_PROPERTIES).forEach(function(entity) {
        EXPORTED_PROPERTIES[entity].forEach(function(property) {
            if (test[entity][property] instanceof Array)
                test[entity][property] = test[entity][property].map(toStringOf);
            else 
                test[entity][property] = test[entity][property].toString();
        });
    });
}
...
exports.openfiscaTest = function openfiscaTest(req, res) {
    ...
    var test = createOpenFiscaTest(req.situation, _.pick(req.body, DECORATION_ATTRIBUTES));
