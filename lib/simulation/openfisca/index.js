...
function simulate(situation, callback) {
    var injectedPrestations = _.keys(mapSituation(situation).familles[0]);
    calculate(situation, false, function(err, response) {
        if (err) return callback(err);

        callback(null, reverseMap(response.value[0].familles[0], situation.dateDeValeur, injectedPrestations));
    });
}
