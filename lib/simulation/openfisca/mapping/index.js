...
var getPeriods = function(dateDeValeur) {
    ...
    return {
        ...
        // les 12 derniers mois (pris individuellement) sous forme d'un array
        douzeDerniersMois: _.map(_.range(1, 12 + 1), function(indice_mois) {
            return {
    ...
    _.forEach(mappingSchema, function(definitions, openFiscaKey) {
        ...
        _.forEach(definitions, function(definition) {
            ...
            if (ressourcesByType[srcKey]) {
                ...
                if (result[periods.moisPrecedent.key] && !_.contains(mesAidesEntity.interruptedRessources, srcKey)) {
...
var setNonInjectedPrestationsToZero = function(openFiscaEntity, dateDeValeur) {
    var periods = getPeriods(dateDeValeur);
    var concernedPeriods = periods.douzeDerniersMois.concat([periods.anneeGlissante]);
