...
var applyRessources = function(src, dest, schema, dateDeValeur) {
    var debutAnneeGlissante = moment(dateDeValeur).subtract(1, 'years').subtract(1, 'months');
    var finAnneeGlissante = moment(dateDeValeur);
    var anneeFiscaleNMoins2 = moment(dateDeValeur).subtract(2, 'years').year();
            ...
            _.forEach(ressourcesByType[srcKey], function(item) {
                if (item.periode) {
                    ...
                    if (periode.isAfter(debutAnneeGlissante, 'month') && periode.isBefore(finAnneeGlissante, 'month')) {
                        surAnneeGlissante += fn ? fn(item.montant) : item.montant;
                    }
                }
            });
