...
var getPeriods = function(dateDeValeur) {
    ...
    return {
        ...
        last12Months: _.map(_.range(1, 12 + 1), function(indice_mois) {
            return {
                key: dateDeValeur.subtract(indice_mois, 'months').format('YYYY-MM')
            };
        }),
       rollingYear: {
        ...
        _.forEach(definitions, function(definition) {
            ...
            if (ressourcesByType[srcKey]) {
                ...
                _.forEach(ressourcesByType[srcKey], function(item) {
                    if (item.periode) {
                        ...
                        _.keys(periods).forEach(function (agregatKey) {
                            if (agregatKey != 'referenceFiscalYear' && isPeriodWithin(agregatKey, moment(item.periode, 'YYYY-MM'))) {
                ...
                var rollingYearKey = periods.rollingYear.key,
                    referenceFiscalYearKey = periods.referenceFiscalYear.key;
                if (! situation.ressourcesYearMoins2Captured) {
                    openFiscaEntity[openFiscaKey][referenceFiscalYearKey] = openFiscaEntity[openFiscaKey][rollingYearKey];
                }
            }
        });
    });
...
var setNonInjectedPrestationsToZero = function(openFiscaEntity, dateDeValeur) {
    var periods = getPeriods(dateDeValeur);
    var concernedPeriods = periods.last12Months.concat([periods.rollingYear]);

    _.forEach(concernedPeriods, function(period) {
        ...
        _.forEach(prestations, function(definition, prestationName) {
            openFiscaEntity[prestationName] = openFiscaEntity[prestationName] || {};
            openFiscaEntity[prestationName][period.key] = openFiscaEntity[prestationName][period.key] || 0;
        });
    });
};
...
exports.buildOpenFiscaRequest = function(situation, debug) {
    return {
        ...
        labels: true,
        base_reforms: ['aides_ville_paris'],
        scenarios: [{
            test_case: {
