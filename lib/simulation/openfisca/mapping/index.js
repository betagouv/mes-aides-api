...
exports.adaptSimulationForOpenFisca = function(simulation) {
    ...
    testCase.foyers_fiscaux.forEach(function(foyerFiscal) {
        var ressourceName = 'pensions_alimentaires_versees';
        var pav = {};

        var entityIndividus = [];
        [foyerFiscal.declarants, foyerFiscal.personnes_a_charge].forEach(function(roleEntity) {
            roleEntity.forEach(function(individuId) {
                entityIndividus.push(individuId);
            });
        });

        entityIndividus.reduce(function(aggregatedRessource, id) {
            var individu = individus[id];
            var individuRessource = individu[ressourceName];
            for (var period in individu[ressourceName]) {
                if (! aggregatedRessource[period])
                    aggregatedRessource[period] = 0;
                aggregatedRessource[period] = aggregatedRessource[period] + individuRessource[period];
            }
            delete individu[ressourceName];
            return aggregatedRessource;
        }, {});
        foyerFiscal.pensions_alimentaires_versees = -pav;
    });
