...
function buildOpenFiscaEntity (mesAidesEntity, mappingSchema, situation) {
    ...
    var previousMonthKeys = ['1MonthsAgo', '2MonthsAgo', '3MonthsAgo'].map(function (month) {
    ...
    _.forEach(mappingSchema, function(definition, openfiscaKey) {
        ...
        if (_.isUndefined(value) || params.isNotDated) {
            return;
        }

        result[openfiscaKey] = {};
        result[openfiscaKey][periods.thisMonth] = value;

        if (! params.copyTo3PreviousMonths) {
            return;
        }

        // Some values must be defined for the latest 3 months to get correct results from OpenFisca
        previousMonthKeys.forEach(function(monthKey) {
