...
var applyRessources = function(src, dest, schema, dateDeValeur) {
    var debutAnneeGlissante = moment(dateDeValeur, 'YYYY-MM-DD').subtract(1, 'years').subtract(1, 'months');
    var finAnneeGlissante = moment(dateDeValeur, 'YYYY-MM-DD');
    var anneeFiscaleNMoins2 = moment(dateDeValeur, 'YYYY-MM-DD').subtract(2, 'years').year();
        ...
        if (ressourcesByType[srcKey]) {
            ...
            _.forEach(ressourcesByType[srcKey], function(item) {
                if (item.periode) {
                    ...
                    var periode = moment(item.periode, 'YYYY-MM');

                    if (periode.isAfter(debutAnneeGlissante, 'month') && periode.isBefore(finAnneeGlissante, 'month')) {
                        surAnneeGlissante += item.montant;
                    }
                }
            });
            ...
            if (fn) surAnneeGlissante = fn(surAnneeGlissante);

            if (_.size(result) > 0) dest[destKey] = result;
            if (surAnneeGlissante > 0) dest[destKey][anneeFiscaleNMoins2.toString()] = surAnneeGlissante;
        }
    });
};
