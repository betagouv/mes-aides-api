...
function mapActivity(person, key, openfiscaObj, code) {
    var situationPro = _.find(person.situationsPro, {situation: key});
    if (situationPro) openfiscaObj.activite = code;
}
...
function mapPatrimoine(situation, targetIndividu) {
    ...
    if (patrimoine) {
        ...
        targetIndividu.valeur_locative_immo_non_loue = patrimoine.valeurLocativeImmoNonLoue;
        targetIndividu.valeur_locative_terrains_non_loue = patrimoine.valeurLocativeTerrainNonLoue;
...
function mapIndividus(situation) {
    return _.map(situation.individus, function(individu) {
        ...
        target.enceinte = individu.enceinte;

        // mapActivity(individu, ACTIF_OCCUPÉ, target, 0);
        mapActivity(individu, 'demandeur_emploi', target, 1);
...
function mapLogement(logement, menage) {
    ...
    if (logement.adresse && logement.adresse.codeInsee) {
        menage.depcom = parseInt(logement.adresse.codeInsee);
    }
